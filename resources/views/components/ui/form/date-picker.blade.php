...
}" x-init="currentDate = new Date('{{ $value }}');
if (datePickerValue) {
...
datePickerCalculateDays();" x-cloak>
    ...
    <div class="w-full">
        ...
        <div class="relative">
            <input x-ref="datePickerInput" name="{{ $name }}" value="{{ $value }}" type="text"
                @click="datePickerOpen = !datePickerOpen" x-model="datePickerValue"
                x-on:keydown.escape="datePickerOpen = false" placeholder="Select date" readonly
            ...
            <div x-show="datePickerOpen" x-transition @click.away="datePickerOpen = false"
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div>
                        <button @click="datePickerPreviousMonth()" type="button"
                            ...
                            <svg class="w-6 h-6 text-gray-400 dark:text-gray-400" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M15 19l-7-7 7-7" />
                            </svg>
                    ...
                    <template x-for="(day, dayIndex) in datePickerDaysInMonth" :key="dayIndex">
                        <div class="px-0.5 mb-1 aspect-square">
                            <div x-text="day" @click="datePickerDayClicked(day)"
                                :class="{
                                    'bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-white': datePickerIsToday(
                                        day),
                                    'text-gray-600 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-700':
                                        !datePickerIsToday(day) && !datePickerIsSelectedDate(day),
                                    'bg-gray-800 text-white hover:bg-opacity-75 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-200': datePickerIsSelectedDate(
                                        day)
                                }"
                ...
                <div class="mt-3 text-right">
                    <button type="button" @click="datePickerSetToday"
